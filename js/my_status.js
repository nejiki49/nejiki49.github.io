// kota、mypokename、shape、Lvのいずれかが変更されたときの処理
$("#kota, #mypokename, #pokeShape input[name=shape], #level_selection input[name=Lv]").on("change", function () {
  // kotaのvalueを取得
  ...
  if (mypokeST) {
    ...
    // if (
    //   (pokeShapeVal === 1 && [4, 8, 12].includes(kotaVal)) ||
    //   (pokeShapeVal === 2 && [0, 8, 12].includes(kotaVal)) ||
    //   (pokeShapeVal === 3 && [0, 4, 12].includes(kotaVal)) ||
    //   (pokeShapeVal === 4 && [0, 4, 8].includes(kotaVal))) 




    //   {
    //   const pokeStatus = document.getElementById('mypoke_status');
    //   pokeStatus.innerHTML = "その型と個体値の組み合わせは存在しません。個体値を切り替えてください";
    // } else if (mypokeST) {
      ...
      if (mypokeBaseH == 1)
        mypokeStatusH = 1;
      else mypokeStatusH = Math.floor((mypokeBaseH * 2 + kotaVal + Math.floor(mypokedH / 4)) * lvVal / 100) + lvVal + 10;//ヌケニン判定、自HP
      mypokeStatusA = Math.floor(Math.floor((mypokeBaseA * 2 + kotaVal + Math.floor(mypokedA / 4)) * lvVal / 100 + 5) * mypokenA / 10);//(自A)計算
      mypokeStatusB = Math.floor(Math.floor((mypokeBaseB * 2 + kotaVal + Math.floor(mypokedB / 4)) * lvVal / 100 + 5) * mypokenB / 10);//(自B)
      mypokeStatusC = Math.floor(Math.floor((mypokeBaseC * 2 + kotaVal + Math.floor(mypokedC / 4)) * lvVal / 100 + 5) * mypokenC / 10);//(自C)
      mypokeStatusD = Math.floor(Math.floor((mypokeBaseD * 2 + kotaVal + Math.floor(mypokedD / 4)) * lvVal / 100 + 5) * mypokenD / 10);//(自D)
      mypokeStatusS = Math.floor(Math.floor((mypokeBaseS * 2 + kotaVal + Math.floor(mypokedS / 4)) * lvVal / 100 + 5) * mypokenS / 10);//(自S)
