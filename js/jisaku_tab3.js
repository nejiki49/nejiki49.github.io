...
//   const selectedLv = document.querySelector('input[name="Lv_tab3"]:checked').value;
//   const pokeListToUse = selectedLv === "50" ? array["50"] : array["100"];

//   // ポケモンの選択肢を作成する
//   const myPokeSelect = document.getElementById("mypokename_tab3");
//   const tekiPokeSelect = document.getElementById("tekipokename_tab3");
...
const levelSelection_tab3 = document.getElementById("level_selection_tab3");
const shapeNum_tab3 = document.getElementById("shape-num_tab3");

function updatePokeList() {
  // 選択されたレベルに応じて、ポケモンの選択肢を切り替える
  const selectedLv = document.querySelector('input[name="Lv_tab3"]:checked');
  const selectedLvValue = selectedLv ? selectedLv.value : "50";
  const setShape = shapeNum_tab3.querySelector('input[name="shape-num_tab3"]:checked').value;
...
levelSelection_tab3.addEventListener("change", updatePokeList);
shapeNum_tab3.addEventListener("change", updatePokeList);
// 初期表示時にポケモンの選択肢を更新する
updatePokeList();

const mypokename_tab3 = document.getElementById("mypokename_tab3");
const tekipokename_tab3 = document.getElementById("tekipokename_tab3");

// levelSelection_tab3.addEventListener("change", function () {
//   mypokename_tab3.options.length = 0;
//   tekipokename_tab3.options.length = 0;
//   const changedPref = document.querySelector('input[name="Lv_tab3"]:checked').value;
//   for (let i = 0; i < array[changedPref].length; i++) {
//     const op = document.createElement("option");
//     const value = array[changedPref][i];
//     op.value = value.cd;
//     op.text = value.label;
//     mypokename_tab3.appendChild(op);

//     const op2 = document.createElement("option");
//     op2.value = value.cd;
//     op2.text = value.label;
//     tekipokename_tab3.appendChild(op2);
//   }
// });

// mypokeのvalueが変更された際に呼び出される関数
function updateMypokeIcon() {
  const mypokename_tab3 = document.getElementById("mypokename_tab3").value;
  document.getElementById("mypokeicon_tab3").src =
    "https://veekun.com/dex/media/pokemon/main-sprites/platinum/" + mypokename_tab3 + ".png";
}

// tekipokeのvalueが変更された際に呼び出される関数
function updateTekipokeIcon() {
  const tekipokename_tab3 = document.getElementById("tekipokename_tab3").value;
  document.getElementById("tekipokeicon").src =
    "https://veekun.com/dex/media/pokemon/main-sprites/platinum/" + tekipokename_tab3 + ".png";
}

// mypokeとtekipokeのchangeイベントにそれぞれ関数を割り当てる
document.getElementById("mypokename_tab3").addEventListener("change", updateMypokeIcon);
document.getElementById("tekipokename_tab3").addEventListener("change", updateTekipokeIcon);

const HGSSPt_tab3 = document.getElementById("HGSSPt_tab3");
const trainers_tab3 = document.getElementById("trainers_tab3");

function updateTrainers() {
  const setLevel = HGSSPt_tab3.querySelector('input[name="HGSSPt_tab3"]:checked').value;
  const setShape = shapeNum_tab3.querySelector('input[name="shape-num_tab3"]:checked').value;
  ...
  trainers_tab3.innerHTML = options;
}

shapeNum_tab3.addEventListener("change", updateTrainers);
HGSSPt_tab3.addEventListener("change", updateTrainers);
